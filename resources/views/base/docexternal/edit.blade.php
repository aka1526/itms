        ...
        <div class="">
            ...
            <div class="row">
                <div class="col-md-12 col-sm-12 ">
                  <div class="x_panel">
                        <div class="x_title">
                            <h2>กำหนดหัวข้อเอกสารภายนอก <small>กรุณากรอกข้อมูล</small></h2>
                            <div class="clearfix"></div>
                        </div>
                        <div class="x_content">


                            <form id="form-post" name="form-post" action="/base/docexternal/update" data-parsley-validate enctype="multipart/form-data" method="POST">
                               @csrf
                                <input type="hidden" id="UNID"  name="UNID" value="{{ $datarow->UNID}}" />


                                <label for="DOC_CODE">ประเภทเอกสาร :</label>
                                <input type="text" id="DOC_CODE" class="form-control" name="DOC_CODE" value="{{ $datarow->DOC_CODE}}" placeholder="ประเภทเอกสาร" required />

                                <label for="DOC_NAME">หัวข้อเอกสารภายนอก:</label>
                                <input type="text" id="DOC_NAME" class="form-control" name="DOC_NAME" value="{{ $datarow->DOC_NAME}}" placeholder="หัวข้อเอกสารภายนอก" required />

                                <label for="DOC_INDEX">จัดลำดับ :</label>
                                <input type="number" id="DOC_INDEX" class="form-control" name="DOC_INDEX" min="0" value="{{ $datarow->DOC_INDEX}}" placeholder="ลำดับ" required />


                              </br>
                                <label>ระบุอื่นๆ * :</label>
                                <p>
                                    <input type="radio" class="flat" name="DOC_OTHER" id="DOC_OTHER_Y" value="Y" {{ $datarow->DOC_OTHER =="Y" ? 'checked=""' : ''}} /> มี
                                    <input type="radio" class="flat" name="DOC_OTHER" id="DOC_OTHER_N" value="N"   {{ $datarow->DOC_OTHER =="N" ? 'checked=""' : ''}} required/> ไม่มี
                                </p>
                                ...
                                <p>
                                    ...
                                    <input type="radio" class="flat" name="DOC_STATUS" id="DOC_STATUS_Y" value="Y"  {{ $datarow->DOC_STATUS =="Y" ? 'checked=""' : ''}}  required /> เปิดใช้งาน
                                    <input type="radio" class="flat" name="DOC_STATUS" id="DOC_STATUS_N" value="N" {{ $datarow->DOC_STATUS =="N" ? 'checked=""' : ''}} /> ปิดใช้งาน
                                </p>
                                 <br />
