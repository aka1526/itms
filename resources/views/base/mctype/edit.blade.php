        ...
        <div class="">
            ...
            <div class="row">
                <div class="col-md-12 col-sm-12 ">
                  <div class="x_panel">
                        <div class="x_title">
                            <h2>กำหนดประภทเครื่องจักร<small>กรุณากรอกข้อมูล</small></h2>
                            <div class="clearfix"></div>
                        </div>
                        <div class="x_content">

                            <!-- start form for validation -->
                            <form id="form-post" name="form-post" action="/base/mctype/update" data-parsley-validate enctype="multipart/form-data" method="POST">
                               @csrf
                                <input type="hidden" id="UNID"  name="UNID" value="{{ $datarow->UNID}}" />


                                <label for="MCTYPE_CODE">รหัสประภทเครื่องจักร :</label>
                                <input type="text" id="MCTYPE_CODE" class="form-control" name="MCTYPE_CODE" value="{{ $datarow->MCTYPE_CODE}}" placeholder="รหัสหน่วยงาน" required />

                                <label for="MCTYPE_NAME">ชื่อประภทเครื่องจักร :</label>
                                <input type="text" id="MCTYPE_NAME" class="form-control" name="MCTYPE_NAME" value="{{ $datarow->MCTYPE_NAME}}" placeholder="ชื่อหน่วยงาน" required />
                                ...
                                <p>
                                    ...
                                    <input type="radio" class="flat" name="MCTYPE_STATUS" id="MCTYPE_STATUS_Y" value="Y"  {{ $datarow->MCTYPE_STATUS =="Y" ? 'checked=""' : ''}}  required /> เปิดใช้งาน
                                    <input type="radio" class="flat" name="MCTYPE_STATUS" id="MCTYPE_STATUS_N" value="N" {{ $datarow->MCTYPE_STATUS =="N" ? 'checked=""' : ''}} /> ปิดใช้งาน
                                </p>
                                 <br />
